// auth-client.js
var API_BASE = (window.NOTIN_API || "").replace(/\/$/, "") || "";
function getPersistedEmail() {
  try {
    return sessionStorage.getItem("notin_email");
  } catch {
    return null;
  }
}
...
var ViewDesc = class {
  ...
  markParentsDirty() {
    ...
    for (let node = this.parent; node; node = node.parent, level++) {
      let dirty2 = level == 1 ? CONTENT_DIRTY : CHILD_DIRTY;
      if (node.dirty < dirty2)
        node.dirty = dirty2;
    }
  }
  get domAtom() {
...
// app.js
var API_BASE2 = (window.NOTIN_API || "").replace(/\/$/, "") || "";
var memToken = null;
var notes = [];
var selectedId = null;
var saveTimer = null;
...
var emptyTrashEl = document.getElementById("emptyTrash");
var searchInput = document.getElementById("searchInput");
var searchClear = document.getElementById("searchClear");
var emptySearchEl = document.getElementById("emptySearch");
var clearSearchEmptyBtn = document.getElementById("clearSearchEmpty");
var newNotebookBtn = document.getElementById("newNotebookBtn");
var newNotebookForm = document.getElementById("newNotebookForm");
var newNotebookInput = document.getElementById("newNotebookInput");
var newNotebookAdd = document.getElementById("newNotebookAdd");
var newNotebookCancel = document.getElementById("newNotebookCancel");
var newNotebookErr = document.getElementById("newNotebookErr");
var notebookListEl = document.getElementById("notebookList");
var nbSelect = document.getElementById("noteNotebookSelect");
var newTagBtn = document.getElementById("newTagBtn");
var newTagForm = document.getElementById("newTagForm");
var newTagInput = document.getElementById("newTagInput");
var newTagAdd = document.getElementById("newTagAdd");
var newTagCancel = document.getElementById("newTagCancel");
var newTagErr = document.getElementById("newTagErr");
...
var newBtn = document.getElementById("newNoteBtn");
var newBtnEmpty = document.getElementById("newNoteBtnEmpty");
var newWrap = document.getElementById("newNoteWrap");
...
var navAll = document.getElementById("navAllNotes");
var navTrash = document.getElementById("navTrash");
...
function getEmail() {
  ...
  if (q) {
    try {
      sessionStorage.setItem("notin_email", q);
    } catch {
    }
...
var PIN_SVG = '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path fill="currentColor" d="M16 9V4h1c.55 0 1-.45 1-1s-.45-1-1-1H7c-.55 0-1 .45-1 1s.45 1 1 1h1v5c0 1.66-1.34 3-3 3v2h5.97v7l1 1 1-1v-7H16v-2c-1.66 0-3-1.34-3-3z"/></svg>';
...
function redirectToLogin() {
  window.location.href = API_BASE2 ? `${API_BASE2}/login.html` : "/login.html";
}
...
async function updateCounts() {
  ...
  try {
    const [activeRes, trashRes] = await Promise.all([
      ...
      fetchWithAuth(API_BASE2 + "/api/notes?filter=trash", { method: "GET" })
    ]);
...
async function loadNotes() {
  ...
  try {
    ...
    if (countEl && (currentQuery || currentNotebookId || currentTagId)) {
      countEl.textContent = currentQuery ? `${notes.length} ${notes.length === 1 ? "match" : "matches"}` : `${notes.length} ${notes.length === 1 ? "note" : "notes"}`;
    }
    if (notes.length === 0) {
      selectedId = null;
      titleInput.value = "";
...
function selectNote(id) {
  ...
  if (!n) return;
  if (layout) {
    layout.classList.remove("is-list");
    layout.classList.add("is-editor");
  }
...
async function createNote() {
  ...
  try {
    ...
    const res = await fetchWithAuth(API_BASE2 + "/api/notes", {
      method: "POST",
...
async function saveNote() {
  ...
  isSaving = true;
  try {
    const res = await fetchWithAuth(API_BASE2 + `/api/notes/${selectedId}`, {
      method: "PUT",
    ...
    updateCounts();
    setTimeout(() => {
      if (!isSaving) setSaveStatus("Saved", "is-saved");
    }, 1200);
  } catch (e) {
...
async function moveToTrash() {
  ...
  try {
    let res = await fetchWithAuth(API_BASE2 + `/api/notes/${selectedId}/trash`, { method: "POST" });
    if (!res.ok) {
      res = await fetchWithAuth(API_BASE2 + `/api/notes/${selectedId}`, { method: "PATCH", body: JSON.stringify({ isTrashed: true }) });
    }
    if (!res.ok) {
      const j = await res.json().catch(() => ({}));
...
async function restoreNote() {
  ...
  try {
    let res = await fetchWithAuth(API_BASE2 + `/api/notes/${selectedId}/restore`, { method: "POST" });
    if (!res.ok) {
      res = await fetchWithAuth(API_BASE2 + `/api/notes/${selectedId}`, { method: "PATCH", body: JSON.stringify({ isTrashed: false }) });
    }
    if (!res.ok) {
      const j = await res.json().catch(() => ({}));
...
async function deleteForever() {
  ...
  try {
    const res = await fetchWithAuth(API_BASE2 + `/api/notes/${selectedId}`, { method: "DELETE" });
    if (!res.ok) {
      const j = await res.json().catch(() => ({}));
...
if (sidebarCollapseBtn) sidebarCollapseBtn.addEventListener("click", () => {
  ...
  try {
    localStorage.setItem("notin_sidebar_collapsed", collapsed ? "1" : "0");
  } catch {
  }
});
try {
  if (localStorage.getItem("notin_sidebar_collapsed") === "1") {
...
function confirmDeleteNotebook(row, nb) {
  ...
  row.querySelector(".app-nb-yes").addEventListener("click", async () => {
    setError("");
    try {
      const res = await fetchWithAuth(API_BASE2 + `/api/notebooks/${nb.id}`, { method: "DELETE" });
      if (!res.ok) {
        const j = await res.json().catch(() => ({}));
...
async function submitNotebook() {
  ...
  try {
    const res = await fetchWithAuth(API_BASE2 + "/api/notebooks", { method: "POST", body: JSON.stringify({ name }) });
    const j = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(j.message || `Create failed ${res.status}`);
    hideNotebookForm();
    await loadNotebooks();
    if (j.id) selectNotebook(j.id);
  } catch (e) {
    if (newNotebookErr) newNotebookErr.textContent = e.message || "Could not create notebook";
  } finally {
    if (newNotebookAdd) newNotebookAdd.disabled = false;
  }
}
...
if (newNotebookBtn) newNotebookBtn.addEventListener("click", () => {
  newNotebookForm && !newNotebookForm.hidden ? hideNotebookForm() : showNotebookForm();
});
...
function confirmDeleteTag(row, tag) {
  ...
  row.querySelector(".app-nb-yes").addEventListener("click", async () => {
    setError("");
    try {
      const res = await fetchWithAuth(API_BASE2 + `/api/tags/${tag.id}`, { method: "DELETE" });
      if (!res.ok) {
        const j = await res.json().catch(() => ({}));
        throw new Error(j.message || `Delete failed ${res.status}`);
      }
...
async function submitTag() {
  ...
  try {
    const res = await fetchWithAuth(API_BASE2 + "/api/tags", { method: "POST", body: JSON.stringify({ name }) });
    const j = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(j.message || `Create failed ${res.status}`);
    hideTagForm();
    await loadTags();
    if (j.id) selectTag(j.id);
  } catch (e) {
    if (newTagErr) newTagErr.textContent = e.message || "Could not create tag";
  } finally {
    if (newTagAdd) newTagAdd.disabled = false;
  }
}
...
function renderTagChips(note) {
  ...
  noteTags.forEach((t) => {
    ...
    chip.innerHTML = `<span class="app-chip-hash" aria-hidden="true">#</span>${escapeHtml(t.name)}<button type="button" class="app-chip-x" title="Remove tag" aria-label="Remove tag ${escapeHtml(t.name)}">\xD7</button>`;
...
if (newTagBtn) newTagBtn.addEventListener("click", () => {
  newTagForm && !newTagForm.hidden ? hideTagForm() : showTagForm();
});
...
if (tagAddSelect) tagAddSelect.addEventListener("change", async () => {
  if (!selectedId || !tagAddSelect.value) {
    if (tagAddSelect) tagAddSelect.value = "";
    return;
  }
  const cur = notes.find((n) => n.id === selectedId);
...
async function togglePin(id) {
  ...
  try {
    const res = await fetchWithAuth(API_BASE2 + `/api/notes/${id}`, {
      method: "PUT",
...
if (clearSearchEmptyBtn) clearSearchEmptyBtn.addEventListener("click", () => {
  clearSearchNow();
  if (searchInput) searchInput.focus();
});
...
if (exportDataBtn) exportDataBtn.addEventListener("click", async () => {
  ...
  try {
    const res = await fetchWithAuth(API_BASE2 + "/api/users/me/export", { method: "GET" });
    if (!res.ok) {
      const j = await res.json().catch(() => ({}));
...
if (deleteAccountBtn) deleteAccountBtn.addEventListener("click", async () => {
  ...
  try {
    const res = await fetchWithAuth(API_BASE2 + "/api/users/me", {
      method: "DELETE",
...
if (mobileBack) {
  mobileBack.addEventListener("click", () => {
    if (layout) {
      layout.classList.remove("is-editor");
      layout.classList.add("is-list");
    }
  });
}
initEditor();
