...
const required = ['GOOGLE_CLIENT_ID','GOOGLE_CLIENT_SECRET','JWT_ACCESS_SECRET','JWT_REFRESH_SECRET','OTP_PEPPER','SMTP_HOST','SMTP_USER','SMTP_PASSWORD'];
...
const mailer = nodemailer.createTransport({host:env.SMTP_HOST,port:Number(env.SMTP_PORT||465),secure:env.SMTP_SECURE !== 'false',auth:{user:env.SMTP_USER,pass:env.SMTP_PASSWORD}});
...
app.use(helmet()); app.use(express.json({limit:'20kb'})); app.use(cookieParser());
