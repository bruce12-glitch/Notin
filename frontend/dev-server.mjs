// Notin landing-page dev server
// - Serves the static landing (Green + Neon editions) from this directory
// - Proxies /api/* and /auth/* to the unified Notin API (default http://127.0.0.1:5000)
//   so the browser only ever talks to ONE origin (required for live-preview hosts
//   where per-port hostnames differ and cross-port calls fail).
//
// Usage:
...
function serveStatic(req, res) {
  ...
  // Basic Range support for media (hero videos)
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`🌿 Notin landing dev server on http://0.0.0.0:${PORT}`);
  console.log(`   Green edition:  /            (index.html)`);
  console.log(`   Neon edition:   /index-neon.html`);
