...
The file needs no edits — it is complete and was validated locally. Once it
lands on a branch, GitHub runs it on every `push` and `pull_request` to `main`.
...
| `playwright install --with-deps chromium` | The browser the Arena sandbox cannot download |
...
| `npm run test:e2e` | Whole `tests/e2e` directory, including the browser journey |
