...
> (`arena/01a0123a-notin`). `migrate.js` is the live source of truth.
> `prisma/schema.prisma` is documentation only — the app does **not**
> run `@prisma/client`.
...
You are the implementation agent for **Notin**. Shipped: notes engine, auth,
attachments, shares, WP-AI-001/002/002b, WP-FUNNEL-001. Next product work is
WP-AI-003 (chat) then deploy — but **this session is schema-only**.
...
├── prisma/notin.sqlite      ← local fallback file (do not commit changes)
...
| model `Tag` (`id`, `userId`, `name`, `createdAt`, `Tag_userId_idx`) | WP-APP-006 |
...
isTrashed/trashedAt/userId/timestamps), `Attachment`, `NoteShare`,
...
- SQLite uses `INTEGER 0/1` for booleans and `TEXT` ISO timestamps. Prisma
  ...
  schema file.
- IDs are `TEXT` / `String @id`. Some tables have `DEFAULT cuid()` in Postgres
  only; JS still generates ids. Keep `@default(cuid())` only where the current
  file already does (User, Note). Attachment / NoteShare / auth tables stay
  `@id` without a default (JS supplies the id).
...
6. Blockers:                [any]
7. Suggested next:          WP-AI-003 (if not shipped) or WP-DEPLOY-001 — do NOT start it.
```

## FUTURE QUEUE (context only — DO NOT BUILD IN THIS SESSION)

1. **WP-AI-003** — chat with note (`CODING_AGENT_MASTER_PROMPT_WP-AI-003.md`)
   if that session has not run yet. Product work is independent of this sync.
2. **WP-DEPLOY-001** — fail-closed production boot, CORS lock, no SQLite
   fallback in `NODE_ENV=production`, CI + Playwright Chromium.
3. **WP-AI-004** — writing assistant (continue / rephrase / shorten).
