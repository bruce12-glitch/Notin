...
| Field | Value |
|---|---|
| **Last Updated** | 2026-08-20 (CTO live audit + **WP-AI-005 shipped** — per-user AI rate limiting, defect found and fixed the same day) |
| **Current Phase** | Phase 2 (AI Layer) — WP-AI-005 defect repair landed 2026-08-20. Phase 2 **complete — WP-AI-001/002/002b/003/003b/004/004b**; WP-SCHEMA-001 mirror, WP-DEPLOY-001 gates, WP-FUNNEL-001, and **WP-LEFTOVERS-001** complete |
| **MVP Completion** | ~81% (unchanged — audit found no missing features, one broken one) |
| **Production readiness** | **~80%** (was ~70% while WP-AI-005 was open; the multi-user launch blocker is now fixed) — fail-closed boot, CORS lock, CSRF/origin guards and the backup/restore drill are all real and were re-verified live on 2026-08-20. The per-IP AI rate-limit blocker (WP-AI-005) is **fixed and verified live**. CI is still inactive (`ci/e2e.yml` not yet moved by a human). |
...
- ❌ **DEFECT FOUND** — per-IP AI rate limiting. **Fixed the same day by WP-AI-005** (see technical debt).
...
- → ~~🚨 **AI rate limiters are keyed by IP, not by user**~~ — **FIXED 2026-08-20 by WP-AI-005.** All five limiters now pass `keyGenerator: aiUserKey` (`u:${req.userId}`, with the IPv6-safe `ipKeyGenerator` fallback). Verified live: user A exhausts its budget (`200×5, 429`), a brand-new user's first call returns **200**, and an exhausted user cannot escape by spoofing `X-Forwarded-For` (still 429). `chatLimit` remains ONE shared instance across `/chat` and `/chat/stream`. New regression spec `ai-ratelimit-smoke.spec.js` fails on the old code at the exact assertion (`Expected 200, Received 429`) and passes on the new. `ai-assist-smoke.spec.js` was restructured: it previously split its budget by spoofing four `X-Forwarded-For` IPs — a trick per-user keying deliberately kills — so its guard assertions now run as two extra signed-up accounts. **Resolved**
- → *(historical, for context)* The original defect: All five `rateLimit(...)` instances in `backend/src/routes/noteRoutes.js` (lines ~35–51) omit `keyGenerator`, so express-rate-limit v8 falls back to client IP. Reproduced live 2026-08-20 on a clean DB: user A burns the summarize budget (`200 200 429 429 429`), then a **brand-new user's first ever AI call returns 429**. Behind a proxy, NAT, campus, or mobile carrier, five requests lock out every other customer on that egress IP for 15 minutes. It also makes the E2E suite order-dependent (re-running the AI specs inside one window fails four of them; `ai-assist-smoke.spec.js:53` documents the workaround instead of catching the bug). Fix = one shared `keyGenerator` on `req.userId` — `auth` already runs before every limiter on that router. **Full agent instruction ready at `AGENT_INSTRUCTION_WP-AI-005.md`.** **Critical**
...
- → AI limiters: all five keyed per **user** (`u:${req.userId}`) since WP-AI-005 — not per IP.
...
1. ✅ ~~**WP-AI-005 — per-user AI rate limiting**~~ — **DONE 2026-08-20.** Shipped with a regression spec; request-only suite is **9 passed** on a fresh DB.
