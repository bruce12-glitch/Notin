...
- → **WP-SEC-002 (2026-08-19):** signed CSRF + trusted-origin enforcement on cookie-carried auth mutations — **PR #2 salvage item #2**. Architectural scope: exactly TWO endpoints authenticate by cookie (`POST /auth/refresh`, `POST /auth/logout`, each mounted at `/api/auth` + legacy `/auth`) — every other route authorizes via the Bearer header, which cross-site pages cannot forge, so those routes stay untouched BY DESIGN. Layer A (trusted origin): mutating methods on the auth router with a present-but-non-allowlisted `Origin` → `403 {"error":"Invalid origin"}`; absent Origin (curl, Playwright request specs) passes — mirrors the WP-DEPLOY-001 allowlist, now single-sourced in `backend/src/lib/httpSecurity.js` (`isOriginAllowed`, used by BOTH the CORS echo and the router `originGuard`; no second constant, source stays `APP_ORIGIN`). Layer B (signed double-submit CSRF): non-httpOnly `notin_csrf` cookie (`rand.hmac`, HMAC-SHA256 keyed by a sha256 derivative of the refresh secret, `timingSafeEqual` verify — zero deps) minted at all four refresh mints (signup/signin/otpVerify + the SEC-001 rotate/grace-sibling tail), rotated with every refresh, cleared at logout AND the replay-nuke path. On refresh/logout the guard requires: cookie present ∧ `x-notin-csrf` header present ∧ equal ∧ signature valid, else `403 {"error":"Invalid CSRF token"}`; no refresh cookie → skip (the SEC-001 generic `401 {"error":"Invalid session"}` path owns it) — guards ordered `strict` → `originGuard` → `csrfGuard` on the two routes. Client `app.js` echoes the cookie via new `readCookie`/`csrfHeaders` helpers at all four fetch sites (both refresh fallbacks + both logout fallbacks); CORS preflight `Access-Control-Allow-Headers` now includes `X-Notin-CSRF`. The `/api/auth/signup|signin` aliases mounted directly in server.js bypass the auth router by design — they read no cookies. New request-only `auth-csrf.spec.js` (bad/dev origin, missing/mismatched/forged-equal CSRF, genuine dual-cookie rotation, logout matrix, cookie-less 401) + `auth-refresh-replay.spec.js` updated to echo the header on every cookie-carrying call (replay semantics unchanged — now provably reachable only with a valid CSRF pair). Shell cache v14→v15 ✅ — **PR #2 salvage scoreboard: 1 ✅ · 2 ✅ · remaining 3 lockout · 4 token-versioning · 5 device inventory · 6 password policy · 7 Express 5**
- → **WP-OPS-001 (2026-08-20):** production ops gate — `GET /health` stays a DB-free liveness probe; `GET /api/health` is a readiness check (`SELECT 1`, 2s timeout, 503 when unreachable, no driver leak) that **cannot** trip the dev-mode Postgres→SQLite downgrade because it calls `pool.query` / `sqliteDb.prepare` directly instead of `query()`. Optional `GET /api/health/deep` adds upload-dir writability (no new deps). Every request gets `req.id` + `X-Request-Id` (sane inbound echoed; hostile replaced). Controllers log via `logError(req, error, context)`; unhandled 500s add additive `requestId`. `SIGTERM`/`SIGINT` drain up to 10s then `db.$disconnect()`, single-flight, force-exit 1 on timeout. `RUNBOOK.md` documents LB vs liveness, 503 first-checks, and tracing by request id. Dedicated request-only `ops-health-smoke` E2E ✅
- → **WP-SEC-003 (2026-08-20):** account lockout / OTP issue throttle — **PR #2 salvage item #3**. Availability-preserving design: a locked password account still runs bcrypt; a correct password succeeds and clears the ladder, while every miss sees `429` with `Retry-After`, so knowing a victim's email cannot keep the real user out (wrong guesses still cost a request and extend escalation). The single source of truth is `throttle.js`: five consecutive failures advance 1 → 5 → 15 → 60 minutes, capped at 60; OTP issue requests allow 5 per email per 15-minute sliding window. The two fixed surfaces are password signin (`/api/users/signin` plus `/api/auth/signin`) and OTP challenge issuance (`otpResend` plus dev `otpDemoRequest`). The deleted-history insight is that `issueOtp` deletes the previous challenge on every issue, so a per-challenge attempts cap never accumulates; the throttle must therefore count issuance by email. Lockout/throttle signals are only `429` plus integer `Retry-After`; existing `404 User not found`, `401 Invalid credentials`, passwordless response, and OTP anti-enumeration `{ ok: true }` contracts do not shift except where the new known-email 429 intentionally yields. Unknown signin emails create no throttle rows and remain the pre-existing enumeration debt; random-email demo spam is bounded by the existing per-IP `strict` limiter, and distributed multi-IP brute force is softened by the per-email ladder but not eliminated. No shell asset changed; cache remains `notin-shell-v15`. **PR-SEC scoreboard: 1 ✅ · 2 ✅ · 3 ✅ · remaining 4 token-versioning · 5 device inventory · 6 password policy · 7 Express 5**

- → Marketing: Green/Neon editions, video/Lottie hero, responsive ✅

## IN PROGRESS

- → None code-wise. WP-OPS-001 (this session) is the last listed production-readiness code gate. Next in queue: hosting (`RUNBOOK.md` with real secrets) once the owner merges the stack.
