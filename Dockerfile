...
ENV NODE_ENV=production PORT=5000 STORAGE_PROVIDER=local
# S3 env (set at runtime for S3 mode): S3_BUCKET, AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, S3_ENDPOINT, S3_FORCE_PATH_STYLE
WORKDIR /app
...
USER notin
EXPOSE 5000
# Liveness probe: process up (never DB). Readiness for LB is GET /api/health (SELECT 1 + uploads writability via storage abstraction)
